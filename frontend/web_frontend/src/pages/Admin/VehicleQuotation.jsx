...
const VehicleQuotation = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            uploadData.append('vehicleId', formData.vehicleId.trim());
            uploadData.append('price', parseFloat(formData.price));
            uploadData.append('quotationPdf', formData.quotationPdf);

            // API call to your backend
            const response = await fetch('http://localhost:8080/api/quotation/create', {
                method: 'POST',
                body: uploadData,
            });
            ...
            if (response.ok) {
                ...
                setSuccess(`Quotation created successfully! ID: ${result.quotationId}`);
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={handleInputChange}
                                    placeholder="Enter the quotation price"
                                    min="0"
                                    step="0.01"
                                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-700"
                                    required
                            ...
                            <div className="pt-4">
                                <button
                                    type="submit"
                                    disabled={loading || !formData.pendingTripId.trim() || !formData.vehicleId.trim() || !formData.price || !formData.quotationPdf}
                                    className={`w-full py-3 px-6 font-semibold rounded-lg transition-all duration-200 ${
                                        loading || !formData.pendingTripId.trim() || !formData.vehicleId.trim() || !formData.price || !formData.quotationPdf
