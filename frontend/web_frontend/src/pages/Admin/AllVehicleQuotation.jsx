...
const AllVehicleQuotation = () => {
    ...
    const API_BASE_URL = "http://localhost:8080/api/quotation";
    ...
    const loadQuotations = () => {
        ...
        if (storedQuotations) {
            setVehicleQuotations(JSON.parse(storedQuotations));
        }
        setLoading(false);
    };
    ...
    const handleSelectQuotation = (quotation) => {
        ...
        alert(`Selected: ${quotation.vehicleId} - ${formatPriceLKR(quotation.price)}`);
    };
                        ...
                        <div className="flex items-center justify-between mb-8 md:mb-12">
                            <div>
                                <h1 className="font-bold text-2xl md:text-3xl text-gray-800 mb-2">
                                    {tripData?.title || "Trip"} <span className="text-gray-500 font-normal text-lg">(All vehicles quotations)</span>
                                </h1>
                                    ...
                                    >
                                        <div className="flex-1 w-full mb-3 sm:mb-0">
                                            ...
                                            <div className="text-sm text-gray-600 mb-1">
                                                {formatPriceLKR(vehicle.price)}
                                            </div>
                                            {vehicle.pdfFilename && (
                                                <div className="text-xs text-gray-500">
                                                    PDF: {vehicle.pdfFilename}
                                                </div>
                                            )}
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <button 
                                                onClick={() => handleViewQuotation(vehicle.quotationId)}
