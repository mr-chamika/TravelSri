...
const VehicleQuotationDetailsModal = ({ isOpen, onClose, quotation }) => {
    ...
    const handleViewVehicleDetails = async () => {
        ...
        try {
            ...
            console.log("Vehicle ID:", quotation.vehicleId);

            const response = await axios.get(`http://localhost:8080/api/vehicles/${quotation.vehicleId}`, {
                headers: {
            ...
            if (error.response?.status === 404) {
                alert('Vehicle not found. The vehicle may have been removed from the system.');
            } else if (error.response?.status === 500) {
                alert('Server error occurred while fetching vehicle details.');
            } else {
                alert('Unable to fetch vehicle details. Please try again later.');
            }
        } finally {
    ...
    return (
        <>
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gray-50 rounded-t-xl">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-800">Vehicle Quotation Details</h2>
                            <p className="text-sm text-gray-600 mt-1">Vehicle ID: {quotation.vehicleId || 'N/A'}</p>
                        </div>
                        <button
                            onClick={onClose}
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="flex items-center space-x-3">
                                        <div className="text-lg font-semibold text-gray-900">{quotation.vehicleId || 'N/A'}</div>
                                        {quotation.vehicleId && (
                                            <button
                        ...
                        <div className="border-t pt-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="bg-green-50 p-4 rounded-lg">
                                    <div className="text-sm text-green-600 font-medium mb-1">Vehicle Type</div>
                                    <div className="text-lg font-semibold text-green-800">{quotation.vehicleType || 'Not specified'}</div>
                                </div>
                            </div>
                        </div>

                        {/* Status History or Timeline could go here */}
                        <div className="border-t pt-6">
                            ...
                            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-medium text-gray-900">{quotation.vehicleId}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-bold text-lg text-purple-600">{formatPriceLKR(quotation.quotedAmount)}</span>
                                </div>
                                <div className="flex justify-between items-center">
            ...
            {/* Vehicle Details Modal (Nested Modal) */}
            <VehicleDetailsModal 
                isOpen={vehicleDetailsOpen}
...
export default VehicleQuotationDetailsModal;